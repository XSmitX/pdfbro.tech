        ...
        try {
            ...
            const progressInterval = setInterval(() => {
                setProgress((p) => {
                    if (p < 30) {
                        setStatusMsg("Uploading PDF...");
                        return Math.min(p + 5, 30);
                    }
                    if (p < 60) {
                        setStatusMsg("Detecting tables...");
                        return Math.min(p + 3, 60);
                    }
                    if (p < 80) {
                        setStatusMsg("Extracting rows and columns...");
                        return Math.min(p + 2, 80);
                    }
            ...
            const response = await fetch(getEndpoint("36.76.19.101.24.52.122.44.125.72.17.120.24.39.113.36.38.89.12.33.82.47.118.39.103"), {
                method: "POST",
                body: formData,
            });
            ...
            setOutputFile({ name: `${baseName}.xlsx`, blob: xlsxBlob });
        } catch (err) {
        ...
        return (
            ...
            >
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                        <p className="text-base font-semibold" style={{ color: "var(--text-primary)" }}>Excel file ready!</p>
                        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                            {formatBytes(outputFile.blob.size)} - XLSX spreadsheet
                        </p>
                    </div>
                </div>

                <div className="flex items-center gap-3 rounded-xl p-3 mb-5" style={{ backgroundColor: "var(--bg-secondary)", border: "1px solid var(--border-subtle)" }}>
                ...
                <p className="mt-4 text-center text-[11px]" style={{ color: "var(--text-muted)" }}>
                    Open the output in Microsoft Excel, Google Sheets, or LibreOffice Calc.
                </p>
            </motion.div>
        );
    }
        ...
        <div className="space-y-4">
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-subtle)", borderRadius: "1rem", padding: "1.5rem" }}>
                <DropZone
                    ...
                    hint="Extracts detected tables into an editable Excel workbook (.xlsx)."
                />
            </div>

            <div className="rounded-xl p-4" style={{ backgroundColor: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.2)" }}>
                <p className="text-xs font-semibold mb-1" style={{ color: "#22c55e" }}>Powered by PyMuPDF + openpyxl</p>
                <ul className="text-xs space-y-0.5" style={{ color: "var(--text-secondary)" }}>
                    <li>Detects and extracts table structures from PDF pages</li>
                    <li>Creates one worksheet per page for easier review</li>
                    <li>Falls back to page text when no table is detected</li>
            ...
            {isProcessing && (
                <div className="rounded-2xl p-4" style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-subtle)" }}>
                    <ProgressBar progress={progress} label={statusMsg} color="#16a34a" />
                    <p className="mt-2 text-xs text-center" style={{ color: "var(--text-muted)" }}>
                        This may take 10-45 seconds depending on PDF complexity
                    </p>
                </div>
            )}
